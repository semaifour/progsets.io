package io.procsets.config.entity;
...
@Component
@EnableMongoRepositories(basePackages = "io.procsets")
public class MongodbConfiguration {
	...
	@PostConstruct
	public void init() {
		String tmp = properties.props().getProperty("procsets.config.store.mongodb.hosts");
		if (tmp != null) {
	...
	public @Bean MongoDbFactory mongoDbFactory() throws Exception {
		if (hosts.size() > 0) {
			...
			return new SimpleMongoDbFactory(new MongoClient(hosts), properties.props().getProperty("procsets.config.store.mongodb.database"));
		} else {
			return null;
		}
	}
