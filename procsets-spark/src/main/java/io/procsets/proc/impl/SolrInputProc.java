...
public class SolrInputProc extends Procedure {
	...
	public Map<String, String> settings(Procontext pc) {
		...
		if (ds == null) {
			ds = pc.appproperties().prop("procsets.proc.isolr.default.datasource", "default");
		}
		//load default settings for the connector
		settings.putAll(pc.appproperties().propsWithPrefix("procsets.proc.isolr.default.solr.", true));
		//load default settings for the data source
		settings.putAll(pc.appproperties().propsWithPrefix("procsets.spark.datasource." + ds + ".solr.", true));
		return settings;
	}
