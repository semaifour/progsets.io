...
public class MongoInputProc extends Procedure {
	...
	public Map<String, String> settings(Procontext pc) {
		...
		if (ds == null) {
			ds = pc.appproperties().prop("procsets.proc.imongo.default.datasource", "default");
		}
		//load default settings for the connector
		settings.putAll(pc.appproperties().propsWithPrefix("procsets.proc.imongo.default.", true));
