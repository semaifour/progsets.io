...
public class CassandraInputProc extends Procedure {
	...
	public Map<String, String> settings(Procontext pc) {
		...
		if (ds == null) {
			ds = pc.appproperties().prop("procsets.proc.cassandra.default.datasource", "defaultcassandra");
		}
		//load default settings for the connector
		settings.putAll(pc.appproperties().propsWithPrefix("procsets.proc.cassandra.default.", true));
