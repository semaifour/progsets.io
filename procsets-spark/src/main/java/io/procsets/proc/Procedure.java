...
public abstract class Procedure implements Serializable {
	...
	/**
	 * Returns procedure settings collection all attrs set with prefix [procsets.proc.<proc-name>.settings.]
	 * 
	 * @return
	 */
	public Map<String, String> settings(Procontext pc) {
		return (HashMap<String, String>)pc.appproperties().propsWithPrefix("procsets.proc." + this.getName() + ".settings.", true).clone();
	}
